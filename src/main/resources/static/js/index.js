...
function sendInfoRequest()
{
    infoXHR.onreadystatechange = function()
    {
        if ((this.readyState == 4) && (this.status == 200))
        {
            ...
            // Handle long numbers by adding CSS classes to the entire grid
            let daysValue = parseInt(response.uptime.days);
            let valuesGrid = document.querySelector('.values-grid');
            let labelsGrid = document.querySelector('.labels-grid');
            let uptimeRectGrid = document.querySelector('.uptime-rectangle-grid');
            
            // Remove existing mode classes from both grids
            if (valuesGrid) {
                valuesGrid.classList.remove('short-mode', 'medium-mode', 'long-mode');
            }
            if (labelsGrid) {
                labelsGrid.classList.remove('short-mode', 'medium-mode', 'long-mode');
            }
            
            // Add appropriate mode class based on days value
            // This will affect all numbers (days, hours, minutes, seconds)
            if (daysValue >= 1000) {
                if (valuesGrid) valuesGrid.classList.add('long-mode');
                if (labelsGrid) labelsGrid.classList.add('long-mode');
            } else if (daysValue >= 100) {
                if (valuesGrid) valuesGrid.classList.add('medium-mode');
                if (labelsGrid) labelsGrid.classList.add('medium-mode');
            } else {
                if (valuesGrid) valuesGrid.classList.add('short-mode');
                if (labelsGrid) labelsGrid.classList.add('short-mode');
            }
            
            // Show the uptime display after styles are applied
            if (uptimeRectGrid && !uptimeRectGrid.classList.contains('initialized')) {
                uptimeRectGrid.classList.add('initialized');
            }
        }
    }
